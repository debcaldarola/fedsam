...
function run_fedsam() {
  echo "############################################## Running FedSAM ##############################################"
  alpha="$1"
  python main.py -dataset cifar10 --num-rounds 10000 --eval-every 100 --batch-size 64 --num-epochs 1 --clients-per-round 5 -model cnn -lr 0.01 --weight-decay 0.0004 -device cuda:0 -algorithm fedopt --server-opt sgd --server-lr 1 --num-workers 0 --where-loading init -alpha ${alpha} --client-algorithm sam -rho 0.5 -eta 0 # TODO change rho eta
}

function run_fedsam_with_swa() {
  echo "############################################## Running FedSAM with SWA ##############################################"
  alpha="$1"
  python main.py -dataset cifar10 --num-rounds 10000 --eval-every 100 --batch-size 64 --num-epochs 1 --clients-per-round 5 -model cnn -lr 0.01 --weight-decay 0.0004 -device cuda:0 -algorithm fedopt --server-opt sgd --server-lr 1 --num-workers 0 --where-loading init -alpha ${alpha} --client-algorithm sam -rho 0.5 -eta 0 -swa --swa-c 10 # TODO change rho eta
}

function run_fedasam() {
  echo "############################################## Running FedASAM ##############################################"
  alpha="$1"
  python main.py -dataset cifar10 --num-rounds 10000 --eval-every 100 --batch-size 64 --num-epochs 1 --clients-per-round 5 -model cnn -lr 0.01 --weight-decay 0.0004 -device cuda:0 -algorithm fedopt --server-opt sgd --server-lr 1 --num-workers 0 --where-loading init -alpha ${alpha} --client-algorithm asam -rho 0.5 -eta 0 # TODO change rho eta
}
